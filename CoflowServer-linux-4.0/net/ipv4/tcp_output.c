...
int coflow_priority_algorithm(unsigned long flow_len, unsigned int Ncfnsh, unsigned int Ncunfnsh, unsigned long coflow_len, unsigned int Nc){
	...
	unsigned long part = (1000*PRIORITY_THRESHOLD + alpha * HC(Ncfnsh,Ncunfnsh,coflow_len,Nc))/(Xf + 1);

	if(part < 1000){
				return ((int)(part * (K-1))/1000 + 1);
...
//	unsigned char id;
	struct coflow *p = NULL;
	struct flow *f = NULL;
//	struct iphdr *iph;
	struct flowi4 *fl4;

	unsigned long long ack_sequence;
/*end*/
	...
	if (tcb->tcp_flags & TCPHDR_ACK && !(tcb->tcp_flags & TCPHDR_FIN)){
		...
		while(p != NULL){
			...
			if(f == NULL) {			
				...
				p->length += ack_sequence - f->ack_seq;
//				printk(KERN_ALERT "p->length: %llu\n", p->length);

				f->ack_seq = ack_sequence;

				flowLen = f->ack_seq - f->syn_ack;	

				//th->res1 = coflow_priority_algorithm(flowLen, p->finished_size, p->size-p->finished_size, p->length, p->size);
				if (th->res1 != coflow_priority_algorithm(flowLen, p->finished_size, p->size-p->finished_size, p->length, p->size)){					
//					printk(KERN_ALERT "flowLen:%lu finished_size:%u unfinished_size:%u length:%llu size:%u\n", flowLen, p->finished_size, p->size-p->finished_size, p->length, p->size);
					th->res1 = coflow_priority_algorithm(flowLen, p->finished_size, p->size-p->finished_size, p->length, p->size);
				}
				/*
				if ( flowLen >= 70){
				...
				*/
				...
				//	printk(KERN_ALERT "taskid:%u length: %u flowLen:%lu [%x,%x,%x, %x] priority:%d===>%d\n", p->taskid, p->length, flowLen, f->saddr, f->daddr, f->source, f->dest, f->priority, th->res1);
				
					f->priority = th->res1;
				}				
				/*haojin: add ACK ip header tos dscp 0x38*/
			//	fl4->flowi4_tos = 0xe0;
