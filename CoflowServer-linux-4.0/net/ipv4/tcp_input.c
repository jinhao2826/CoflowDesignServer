...
extern unsigned long long PRIORITY_THRESHOLD;
struct coflow *coflow_header = NULL;
//spinlock_t coflow_lock = SPIN_LOCK_UNLOCKED; 
//DEFINE_SPINLOCK(coflow_lock);
spinlock_t coflow_lock = __SPIN_LOCK_UNLOCKED(coflow_lock);

unsigned int coflowsum = 0;
unsigned int coflowLessThresholdSum = 0;
unsigned int timeLessThreshold = 0;
/*end*/
...
static void tcp_fin(struct sock *sk)
{
	...
	unsigned long long flow_time;
	unsigned int avgtimeLessThreshold;
	/*end*/
	...
	case TCP_ESTABLISHED:
		...
		while(p != NULL){		
			...
			while (f != NULL){
				...
				if((f->saddr == fl4->daddr) && (f->daddr == fl4->saddr) && (f->source == inet->inet_dport) && (f->dest == inet->inet_sport) )
				{
					break;		
				}
				pref = f;
			...
			if(f == NULL) { 		
				prep = p;
				p = p->next;	
			}else if((f->saddr == fl4->daddr) && (f->daddr == fl4->saddr) && (f->source == inet->inet_dport) && (f->dest == inet->inet_sport)){ 
				if(p->flow == f){
					p->flow = f->next;
					kfree(f);
		...
		//		printk(KERN_ALERT "size:%u finished_size:%u ", p->size, p->finished_size);
		...
		//				printk(KERN_ALERT "co_header: FREE coflow\n");
					}else if(prep != NULL){
						prep->next = p->next;	
		//				printk(KERN_ALERT "FREE coflow\n");
					}
					//get the coflow stop time micro second
					p->stop = jiffies;
					
					coflowsum++;
					flow_time = p->stop - p->start;
					if(p->length <=  PRIORITY_THRESHOLD){
						timeLessThreshold += flow_time;
...
//		printk(KERN_ALERT "The taskid: %x\n", id);
	...
	if (id >=1 && id <128){
		if ((flow1 = kmalloc(sizeof(struct flow),  GFP_KERNEL)) != NULL){
...
//			printk(KERN_ALERT "kmalloc struct coflow\n");
			...
			q->finished_size = 0;
			q->length = 0;
			//get the coflow start micro second
			q->start = jiffies;
			q->stop = 0;

			q->flow = flow1;
//			printk(KERN_ALERT "coflow build\n");
		}else if(p->taskid == id){     /*it is a old coflow*/
//			printk(KERN_ALERT "find the existing taskid\n");
			 	f = p->flow;
				fprev = f;
				while(f){
					fprev = f;
